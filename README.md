...
> - **ライセンスに関する注意**: 本リポジトリのソースコードはオープンソースとして公開されていますが、リポジトリ内に含まれる「企業ロゴ」や「スポンサーロゴ」等の画像アセットは各企業に著作権および商標権が帰属します。これらの画像アセットに関する無断転用や二次利用は固くお断りいたします。
...
スポンサーの追加・変更を行う場合は、以下の手順で作業と確認を行ってください。

1. **ロゴ画像の配置**: 追加するスポンサーのロゴ画像を `src/assets/sponsors/` に保存します。
2. **ファイルの編集**: `src/components/sponsors.astro` を開き、以下の編集を行います。
   - ファイル上部で、保存したロゴ画像をインポートします。
   - 先頭にある `sponsors` 配列のうち、該当するプランの `companies` に、企業名とインポートしたロゴを追加します。
3. **表示の確認**: プルリクエストを作成した際、発行される確認用のアドレス（URL）にアクセスし、追加したロゴが正しいサイズと位置で表示されているかを必ず確認してください。
